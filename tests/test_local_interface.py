...
def test_bilingual_index_is_strict_offline_and_contains_ten_cases(
    tmp_path: Path,
) -> None:
    path = build_index(tmp_path / "index.html")
    text = path.read_text(encoding="utf-8")

    assert text.count('class="case-card"') == 10
    assert "十轮本地验证总览" in text
...
def test_registered_auxiliary_source_runner_is_whitelist_checked(
    ...
    assert not list(
        (ROOT / "examples" / "company-loops" / "albemarle").glob(
