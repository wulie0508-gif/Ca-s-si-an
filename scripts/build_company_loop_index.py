"""Build the offline bilingual index for the registered company loops."""
...
except ModuleNotFoundError:  # Direct execution: python scripts/build_company_loop_index.py
...
def _outcome_badge(outcome: dict[str, str | None]) -> str:
    status = outcome["status"]
    signal = outcome["signal"]
    if status == "not_applicable":
        return '<span class="signal na">N/A / 不适用</span>'
    if status == "not_yet_applicable":
        return '<span class="signal not-yet">Not yet applicable / 暂不适用</span>'
    if status != "evaluated" or signal is None:
        raise ValueError(f"Cannot render unsupported dimension outcome: {outcome}")
    label = {
        "red": "Red / 红色",
        "amber": "Amber / 黄色",
        "green": "Green / 绿色",
...
def _load_cases() -> list[dict[str, Any]]:
    registry = validate_registry_contract(json.loads(REGISTRY.read_text(encoding="utf-8")))
    ...
    for row in registry:
        ...
        if outcomes != row["expected_outcomes"]:
            raise ValueError(
                f"Case '{row['id']}' outcome mismatch: {outcomes} != {row['expected_outcomes']}"
            )
...
def _card(case: dict[str, Any]) -> str:
    profit = case["outcomes"]["profitability-unit-economics"]
    cash = case["outcomes"]["cash-runway"]
    outcome_tokens = " ".join(
        str(outcome["signal"]) if outcome["status"] == "evaluated" else str(outcome["status"])
        for outcome in (profit, cash)
    )
...
<article class="case-card" data-case="{html.escape(case["id"])}" data-company="{html.escape(case["display_name"].lower())}"
  data-aux="{html.escape(case["auxiliary_status"])}" data-outcomes="{outcome_tokens}">
  ...
  <div class="dimension-row"><div><small>Profitability / 盈利</small>{_outcome_badge(profit)}</div><div><small>Cash / 现金</small>{_outcome_badge(cash)}</div></div>
...
def build_index(output: str | Path = DEFAULT_OUTPUT) -> Path:
    ...
    heading_zh = "十轮本地验证总览" if case_count == 10 else f"{case_count} 轮本地验证总览"
    ...
    document = f"""<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>CleanTech Finance — registered company loops</title><style>{STYLE}</style></head><body><main>
<div class="eyebrow">CleanTech Finance · offline deterministic core</div><h1>Registered company-loop validation<span lang="zh-CN">{heading_zh}</span></h1>
<p class="lede">{case_count} registered companies and failure-driven generalization steps, bilingual evidence cards, and user-selectable non-authoritative sources.<span lang="zh-CN">{case_count} 家已注册公司与对应的失败驱动泛化优化、双语证据卡，以及可由用户选择且不具信号权威的辅助来源。</span></p>
...
<section class="stats"><div class="stat"><strong>{case_count}/{case_count}</strong>loops complete / 循环完成</div><div class="stat"><strong>{case_count}</strong>unique entities / 唯一实体</div><div class="stat"><strong>0</strong>model calls / 模型调用</div><div class="stat"><strong>{len(REQUESTED_DIMENSIONS)}</strong>validated dimensions / 已验证维度</div></section>
<section class="controls" aria-label="Filters / 筛选"><label>Search / 搜索<input id="search" type="search" placeholder="Company or case id / 公司或案例编号"></label><label>Outcome / 结果<select id="signal"><option value="all">All / 全部</option><option value="red">Red / 红色</option><option value="amber">Amber / 黄色</option><option value="green">Green / 绿色</option><option value="not_applicable">N/A / 不适用</option><option value="not_yet_applicable">Not yet applicable / 暂不适用</option></select></label><label>Aux status / 辅助状态<select id="aux"><option value="all">All / 全部</option>{aux_options}</select></label><p id="result-count" class="result-count" aria-live="polite"></p></section>
